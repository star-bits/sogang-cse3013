...
Array::Array(int size) {
  ...
  if (size < 0) {
    ...
    // size number of int memory blocks allocated at runtime
    data = new int[size];
    len = size;
  }
}
...
Array::~Array() {
  // 소멸자; 할당된 메모리 해제
  delete[] data;  // deallocates the array; hence the '[]'
}
...
int& Array::operator[](int i)  // 배열에 원소 삽입
{
  ...
  // 배열의 인덱스가 범위 내에 있으면 값 리턴, 그렇지 않으면 에러메세지 출력하고
  // tmp리턴
  if (i >= 0 && i < len) {
    return data[i];
  } else {
...
{
  // 배열의 인덱스가 범위 내에 잇으면 값을 리턴, 그렇지 않으면 에러메세지
  // 출력하고 0을 리턴
  if (i >= 0 && i < len) {
    return data[i];
  } else {
